...
<!-- MB: Jumping straight to multiple categories here rather than just a binary one. But then where do we introduce plogis? -->
...
Now we can see how the effect of `age` varies by `race`.  While the passage of time is associated with higher presidential approval in , `year` is associated with declining Democratic fortunes in the Midwest (`year`) and the South (`regionSouth:year`).
...
x <- house_ch12 %>% 
  ...
  rep_sample_n(size = nrow(house_ch12), replace = TRUE, reps = 4)
...
```
```{r, echo=FALSE}
regression_points <- augment(house_region_model, se_fit = TRUE) %>%
  mutate(conf.low = .fitted - 2 * .se.fit,
         conf.high = .fitted + 2 * .se.fit) %>%
  select(dem_win, region, .fitted, conf.low, conf.high, .resid)
regression_points %>%
...
<!-- Where numeric used to be --> 
<!-- Where numeric and categorical used to be --> 
...
The code to do this is very similar to the code we used for the gubernatorial forecasts in Chapter \@ref(pitfalls) and the Seattle house prices in Chapter \@ref(continuous-response).  However, we will use `glm()` instead of `lm()`.
...
knitr::include_graphics("12-discrete-response/images/Decision-Tree-for-Heart-Attack-Victim-adapted-from-Gigerenzer-et-al-1999-4.png")
```
...
## Plogis and diivide by 4 and binary categorical
