...
For our multi-variable logistic regression, we'll be using 3 different explanatory variables, as well as the interaction between 2 of those variables, `race` and `age`. We will begin by defining the recipe, or formula, of our model. In our last two examples, we constructed our formulas directly in `fit()`. This time, we will do it seperately, as the recipe will be used seperately later when cross-validating our results.
...
One thing is missing from this recipe: the interaction between age and race. `recipe()` does not allow you to include interaction terms right in the formula, so you have to add them using another step:

```{r}
log_recipe <- log_recipe %>%
  step_interact(terms = ~ race:age)
```



```{r}
logistic_fit <- fit(logistic_mod, log_recipe)
...
You may notice that the coefficient associated with each race has changed significantly. Here is the output of `race_model` alone.
...
cart_recipe <- recipe(approval ~ age + race, data = ch12) %>%
  step_normalize(all_numeric()) %>%
  step_knnimpute(all_predictors())
...
Now that we have our model, let's use the `vfold_cv` to cross-validate it. To see the average metrics for all the folds, we can use the `collect_metrics()` function:

```{r}

approval_cv <- vfold_cv(approval_train, v = 3) 

fit_resamples(approval_forest,
              approval_recipe,
              approval_cv) %>%
  collect_metrics()
