...
<!-- MB: Is it a worthwhile discussion to include graphs for roc_auc or -->

`roc_auc` is similar to `accuracy` in that it is a measure of how good a model is at predicting 0s to be 0 and 1s to be 1 in the dataset. However, it is calculated by taking the area under the ROC curve. `roc_auc` itself is a shortened form of "area under the ROC curve." The ROC curve is created by graphing the rate of true positives as the y-axis (those who approved of the President who were labelled as such) against the rate of false positives (those who approved of the President but were labelled as disapproving). The area under this curve comes out to a very similar measure of accuracy. A `roc_auc` of 1 means that the testing set was perfectly predicted while a 0 means that every observation of the testing set was  falsely predicted.
...
There are an infinite numbers of comparisons to be made within this dataset. Let's return to our original question of how Blacks and Hispanics specifically vary in their support of the president. 

First, we'll transform the data to categorize `race` into two groups: Black/Hispanic or other. We'll also code approval to be an integer, which will help when doing the math for our graphs.

```{r}
ch12 <- ch12 %>% 
  mutate(bh = case_when(
    race == "Black" ~ "Black/Hispanic",
    race == "Hispanic" ~ "Black/Hispanic",
    race == "White" ~ "Other",
...
We can get two important pieces of information from this graph. The first observation is that there are far more observations in the non-Black/Hispanic group. The second is there is noticeably stronger disapproval of the President among Blacks and Hispanics than the rest of the population. 
...
ch12 %>%
  ...
  ggplot(aes(x = age, y = approval_rate, group = bh, color = bh)) +
  geom_point() +
  geom_line() + 
  labs(x = "Age",
       y = "Approval Rate",
       title = "Presidential Approval Rate by Race and Age")
```

In keeping with our previous chart, the presidential approval is consistently higher for all ages among non-Blacks and Hispanics. You can also see that the non-Black/Hispanic population increases in their support of the president on average as they get older. Black and Hispanic voters, however continue to hover at around 25% approval, albeit with greater variation. 

Now, let's add another variable by throwing gender in the mix.

```{r, message = FALSE}
ch12 %>%
  group_by(age, bh, gender) %>%
  summarise(approval_rate = sum(approval)/n()) %>%
  ggplot(aes(x = age, y = approval_rate, group = bh, color = bh)) +
  geom_point() +
  geom_line() + 
  ...
  labs(x = "Age",
       y = "Approval Rate",
       title = "Presidential Approval Rate by Race, Age, and Gender")
```

Now that we've faceted by gender, we can see that the trends from the previous graph don't vary much from male to female. You could argue, based on this graph, that presidential support is mildly higher for Black/Hispanic males than Black/Hispanic females. 

Now that we've done some analysis, let's use our models to help answer more questions about how Black and Hispanic voters vary in their support of the President. 
