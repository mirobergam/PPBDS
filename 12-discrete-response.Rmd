...
approval_forest <- fit(forest_mod,
                    factor(approval) ~ ideology + gender + age,
                    data = ch12)
...
```{r}
temp <- randomForest(approval ~ gender + ideology + age, data = ch12, na.action = na.omit, importance = TRUE, ntree = 250)
```


We can see that in this case, the accuracy improves as we add more trees until about 50 trees where accuracy stabilizes.
