...
Now, let's take a look at our ideology variable. `ideology` has values "Very Liberal", "Liberal", "Moderate", "Conservative", "Very Conservative", and "Not Sure". We will make a barplot to get a sense of the distribution.
...
To create classification trees, we'll use the `decision_tree()` model specification and the `"rpart"` engine.  The syntax is very similar to when we used `logistic_reg()`. 
...
cces_ch12 %>%
  ...
  drop_na() %>% #MB: Drop NAs at the beginning to avoid this
  group_by(ideology) %>%
