...
<!-- Questions
- Streamining tidymodels workflow
- tree can't handle interaction terms, so how do I compare the performances?
- tree becomes overly basic when I include a third term
-->
...
You may notice that the coefficient associated with each race has changed significantly from `race_model`. Here is the output of `race_model` alone.
...
<!-- MB: Trees cannot handle interaction terms, so how should I compare the models? -->

```{r}
cart_tree <- fit(tree_mod,
                 cart_formula,
                 cart_training)
```
