...
# Creating a binary presidential approval variable
# Including three as marking approval

cces_ch12 <- cces %>%
  
  mutate(pres_approval = as.integer(case_when(
    approval == 5 ~ 1,
    approval == 4 ~ 1,
    approval == 3 ~ 1,
    approval == 2 ~ 0,
  ...
  select(year, race, ideology, education, pres_approval)
...
cces_ch12 %>%
  ...
  cols_label(
    ...
    pres_approval = md("President Approval?")) %>%
  tab_source_note(md("A random sample of 5 out of the 452,755 individuals surveyed"))
...
```{r}
cces_ch12 %>% 
  select(pres_approval, ideology, year) %>% 
  skim()
```
