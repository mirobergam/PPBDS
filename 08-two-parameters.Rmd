...
From these plots, we can gather that Democrats in this dataset are on average older than the Republicans, with their mode age being 50. Additionally, treated individuals seem to have a higher attitude end than the control group. The control group seems to have an high outlier for attitude end. 
...
The fitted values are the same for all Republicans and for all Democrats. Remember that the actual age of each observation is the addition of the model and "real-world" noise (or the residual). The fitted value represents the "model" part of that equation, which is why each value is one of the two coefficients outputted by our model. 
...
The other half of the equation, the noise, is represented by epilson, which accounts for the difference between each individual's age from its fitted value. Note the new column name for age: $$y_i$$ is the sum of the fitted epilson value. 
