...
<!-- 1) Add a Rubin Table for the age ~ party section. Include discussion of modeling for prediction. -->

<!-- At end of EDA, add a plot of raw data, jittered or whatever to look good. -->

<!-- 2) Explain meaning of regression results in age ~ party. Discuss key themes like parameter uncertainty and unmodeled variation. Also, predicted value and residual. -->

<!-- 3) Send a PR when each of these is done. We can then Zoom and discuss next steps. Much of the chapter is a mess so we need to go step-by-step. -->
...
This table is very simple, which is the point. There is no causal effect to be studied as we are simply trying to predict the immutable variable of age.

This mathematical formula will also help us better understand the model:
$$ y = b_0 + b_1 x $$
$$ y = (42.6) + (41.16) x $$
When the individual is a Democrat (x=0), their age is predicted to be 42.6. When the individual is a Republican (x=1), their age is predicted to be 41.16. 

$$ y_i = b_0 + b_1 x_i + \epsilon_i $$

If you know the age and party affiliation of any single individual, you can solve for epsilson, or the residual, of that observation. This is how much the individual's age varies from the prediction of their age in the model. 

<!-- There's an issue with this formula... should there not be an intercept? But then the math doesn't work for dems -->
...
If a new Democrat entered the train station, how easy would it be to guess their age with our model? Could you confidently say that they would be within the interval bounds of 40 and 45?
